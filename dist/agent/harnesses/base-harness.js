...
export class BaseHarness {
    ...
    buildTaskPrompt() {
        ...
        if (wisdom.conventions.length > 0 ||
            ...
            if (wisdom.successes.length > 0) {
                ...
                for (const item of wisdom.successes.slice(0, 5))
                    lines.push(`- ${item}`);
            }
        }
    ...
    async execute() {
        ...
        while (true) {
            ...
            try {
                ...
                logger.error(`[${this.id}] Inference error (${consecutiveInferenceErrors}/${MAX_CONSECUTIVE_INFERENCE_ERRORS}): ${message}`);
                if (consecutiveInferenceErrors >= MAX_CONSECUTIVE_INFERENCE_ERRORS) {
                    throw new Error(`${MAX_CONSECUTIVE_INFERENCE_ERRORS} consecutive inference failures. Last error: ${message}`);
                }
                continue;
            }
            this.beforeTurn();
            this.context.budget.turnsUsed++;
            logger.info(`[${this.id}] Turn ${this.context.budget.turnsUsed}/${this.context.budget.maxTurns} — task: ${this.task.title.slice(0, 60)}`);
            if (response.toolCalls && response.toolCalls.length > 0) {
                ...
                for (const toolCall of response.toolCalls) {
                    const loopResult = this.loopDetector.recordToolCall(toolCall.function.name, toolCall.function.arguments);
                    if (loopResult.blocked) {
                        logger.warn(`[${this.id}] Loop detected: ${loopResult.reason}`);
                        this.messages.push({
                            role: "tool",
                            content: `LOOP DETECTED: ${loopResult.reason}\n\n` +
    ...
    checkBudget() {
        ...
        if (budget.turnsUsed >= budget.maxTurns) {
            throw new Error(`Budget exhausted: reached max turns (${budget.turnsUsed}/${budget.maxTurns}). ` +
                `Task "${this.task.title}" did not complete within the turn budget.`);
        }
        if (budget.costUsedCents >= budget.maxCostCents) {
            throw new Error(`Budget exhausted: cost limit reached ($${(budget.costUsedCents / 100).toFixed(2)} / $${(budget.maxCostCents / 100).toFixed(2)}). ` +
        ...
        if (budget.startedAt > 0) {
            ...
            if (elapsed >= budget.timeoutMs) {
                throw new Error(`Budget exhausted: timeout (${Math.round(elapsed / 1000)}s / ${Math.round(budget.timeoutMs / 1000)}s). ` +
                    `Task "${this.task.title}" exceeded its time budget.`);
            }
        }
    }
}
//# sourceMappingURL=base-harness.js.map