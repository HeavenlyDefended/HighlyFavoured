...
export class GeneralHarness extends BaseHarness {
    id = "general";
    description = "General-purpose agent for research, web interaction, and non-coding execution tasks.";
    transferToolCallCount = 0;
    buildSystemPrompt() {
...
- Any important findings or data discovered`;
        ...
        const customTools = [
            {
                ...
                execute: async (args) => {
                    ...
                    const timeoutMs = Math.min(typeof args.timeout_ms === "number" ? args.timeout_ms : MAX_EXEC_TIMEOUT_MS, MAX_EXEC_TIMEOUT_MS);
    ...
    createWrappedTool(tool, toolCatalog) {
        return {
            name: tool.name,
            ...
            execute: async (args) => {
                ...
                const result = await executeTool(tool.name, args, toolCatalog, this.context.toolContext, this.context.policyEngine, {
    ...
    createSpecAliasTools(toolCatalog, reservedToolNames) {
        return Object.entries(GENERAL_SPEC_ALIAS_TARGETS)
            ...
            return [{
                    ...
                    execute: async (args) => {
                        ...
                        const result = await executeTool(targetName, args, toolCatalog, this.context.toolContext, this.context.policyEngine, {
                            inputSource: this.context.inputSource,
                            turnToolCallCount: this.transferToolCallCount,
                            sessionSpend: this.context.spendTracker ?? NOOP_SPEND_TRACKER,
                        });
...
function confineToWorkspace(filePath, allowedRoot) {
...
function formatExecResult(stdout, stderr) {
...
//# sourceMappingURL=general-harness.js.map