...
import { createPlannerFailureFromVerification, createPlannerGoalFromTask, planGoal, replanAfterFailure, taskToPlannerFailureInput, } from "../../orchestration/planner.js";
import { buildPlannerContext, getCurrentPlannerVersion, persistPlannerArtifacts, } from "../../orchestration/planner-context.js";
import { decomposeGoal, normalizeTaskResult, } from "../../orchestration/task-graph.js";
import { BaseHarness } from "./base-harness.js";
const MAX_FIX_CYCLES = 3;
export class OrchestratorHarness extends BaseHarness {
    id = "orchestrator";
    description = "Orchestrator agent for task decomposition, delegation, verification, and fix coordination.";
    delegatedTaskIds = [];
    currentPlan = null;
    planVersion = 0;
    plannerMode = "plan";
    plannerError = null;
    planTaskIdByIndex = new Map();
...
- If you have nothing left to delegate or verify, call task_done.`;
    ...
    buildTaskPrompt() {
        ...
        if (this.currentPlan) {
            sections.push(`Current planner mode: ${this.plannerMode}`, `Current planner version: v${this.planVersion}`, formatPlanSummary(this.currentPlan));
        }
        ...
        sections.push("", "## Operating Instructions", "- Start with refresh_plan if you need the latest planner output.", "- Use delegate_task(plan_task_index=...) to materialize planned tasks in order.", "- Use dependency task IDs returned by delegate_task for any manual follow-up tasks.", "- Verify completed work before declaring success.", "- If a delegated task fails or verification fails, call refresh_plan again to get a planner-backed fix plan.");
        ...
        return [
            ...
            },
            {
                ...
                execute: async (args) => {
                    try {
                        ...
                        const createdTask = getTasksByGoal(this.context.db, this.context.goalId).find((task) => !beforeIds.has(task.id)
            ...
            },
            {
                ...
                execute: async (args) => {
                    ...
                    if (!task || task.goalId !== this.context.goalId)
            ...
            },
            {
                ...
                execute: async (args) => {
                    ...
                    const result = normalizeTaskResult(task.result);
                    if (!result)
                        return `Task ${taskId} has no result data`;
            ...
            },
            {
                ...
                execute: async (args) => {
                    ...
                    if (task.status !== "completed")
                        return `Task ${taskId} is not completed (status: ${task.status})`;
                    const result = normalizeTaskResult(task.result);
                    if (!result)
                        return `Task ${taskId} has no result data`;
    ...
    async refreshPlanInternal(params) {
        this.refreshDelegatedTaskState();
        const plannerContext = await buildPlannerContext({
            db: this.context.db,
            workspace: this.context.workspace,
            usdcBalance: Number(this.context.config?.usdcBalance ?? 0),
        ...
        const plan = failureInput
            ? await replanAfterFailure(planningGoal, failureInput, plannerContext, this.context.inference)
            : await planGoal(planningGoal, plannerContext, this.context.inference);
        ...
        this.context.workspace.logDecision(decision, params.failureNote ?? plan.strategy, this.task.agentRole ?? "orchestrator");
    ...
    shouldRefreshPlanOnInitialize() {
        if (this.getManagedTasks().some((task) => task.status === "failed")) {
            return true;
        }
        return this.getManagedTasks().length === 0;
    }
    refreshDelegatedTaskState() {
        const managedTasks = this.getManagedTasks();
        this.delegatedTaskIds = managedTasks.map((task) => task.id);
        this.planTaskIdByIndex = new Map([...this.planTaskIdByIndex.entries()].filter(([, taskId]) => managedTasks.some((task) => task.id === taskId && !isTerminalTaskStatus(task.status))));
    }
    getManagedTasks() {
    ...
    getPlanWorkspacePath() {
        return path.join(this.context.workspace.basePath, "subplans", this.task.id);
    }
    findManagedTaskBySignature(title, role, description) {
        const normalizedTitle = normalizeText(title);
        const normalizedRole = normalizeText(role);
        const normalizedDescription = normalizeText(description);
        return this.getManagedTasks().find((task) => !isTerminalTaskStatus(task.status)
    ...
    resolvePlannedDependencies(dependencies) {
        return dependencies
            .map((index) => this.planTaskIdByIndex.get(index))
            .filter((value) => typeof value === "string");
    }
    trackDelegatedTask(taskId, plannedIndex) {
        if (taskId === "unknown") {
            return;
        }
...
function formatPlanSummary(plan) {
    ...
    plan.tasks.forEach((task, index) => {
        lines.push(`${index}. ${task.title} [role=${task.agentRole}, priority=${task.priority}, deps=${task.dependencies.length > 0 ? task.dependencies.join(", ") : "none"}]`, `   ${task.description}`);
    });
    return lines.join("\n");
}
function formatManagedTask(task) {
    const result = normalizeTaskResult(task.result);
...
function buildPlanTaskIndexMap(plan, managedTasks) {
    const remainingTasks = managedTasks.filter((task) => !isTerminalTaskStatus(task.status));
    const mapping = new Map();
    plan.tasks.forEach((plannedTask, index) => {
        const matchIndex = remainingTasks.findIndex((task) => normalizeText(task.title) === normalizeText(plannedTask.title)
...
//# sourceMappingURL=orchestrator-harness.js.map