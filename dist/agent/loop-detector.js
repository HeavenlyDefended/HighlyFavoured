...
export class LoopDetector {
    ...
    turnPatterns = [];
    currentTurnTools = [];
    patternWarningIssued = null;
    consecutiveIdleOnlyTurns = 0;
    currentTurnIsIdleOnly = true;
    constructor(config) {
        this.config = {
    ...
    recordToolCall(name, args) {
        ...
        if (this.callHistory.length >= threshold) {
            ...
            const allIdentical = recent.every((call) => call.name === name && call.argsHash === argsHash);
            if (allIdentical) {
                return {
                    blocked: true,
                    reason: `You have called "${name}" with identical arguments ${threshold} times in a row. ` +
    ...
    endTurn() {
        ...
        if (this.turnPatterns.length >= 3) {
            ...
            if (allSame) {
                if (this.patternWarningIssued === pattern) {
                    ...
                    return {
                        blocked: true,
                        reason: `LOOP ENFORCEMENT: You were warned about repeating the tool pattern "${pattern}" ` +
                ...
                return {
                    blocked: false,
                    reason: `WARNING: You have repeated the tool pattern "${pattern}" for 3 consecutive turns. ` +
        ...
        if (this.consecutiveIdleOnlyTurns >= this.config.maxIdleOnlyTurns) {
            ...
            return {
                blocked: false,
                reason: `IDLE LOOP DETECTED: Your last ${this.config.maxIdleOnlyTurns} turns only used ` +
