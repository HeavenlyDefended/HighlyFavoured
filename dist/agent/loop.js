    ...
    while (running) {
        ...
        try {
            ...
            if (orchestrator) {
                ...
                const hasSelfAssignedParentTask = !!db.raw.prepare(`SELECT 1 FROM task_graph WHERE assigned_to = ? AND status IN ('assigned', 'running') LIMIT 1`).get(identity.address);
                if (orchestratorTick.phase === "executing" &&
                    ...
                    log(config, "[ORCHESTRATOR] All delegated work is active and no self-assigned parent task remains. Sleeping to avoid idle loop.");
...
async function getFinancialState(conway, address, db, chainType) {
    ...
    try {
        ...
        usdcBalance = await getUsdcBalance(address, network, chainType);
        if (usdcBalance > 0)
