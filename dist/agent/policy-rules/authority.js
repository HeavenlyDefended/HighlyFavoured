 ...
 */
function isExternalSource(inputSource) {
 ...
 */
function createExternalToolRestrictionRule() {
    return {
        ...
        evaluate(request) {
            if (isExternalSource(request.turnContext.inputSource)) {
                return deny("authority.external_tool_restriction", "EXTERNAL_DANGEROUS_TOOL", `External input (source: ${request.turnContext.inputSource ?? "undefined"}) cannot use dangerous tool "${request.tool.name}"`);
            }
            return null;
        },
    };
 ...
 */
function createSelfModFromExternalRule() {
    return {
        ...
        evaluate(request) {
            ...
            for (const protectedPath of PROTECTED_PATHS) {
                if (normalizedPath.includes(protectedPath.toLowerCase())) {
                    return deny("authority.self_mod_from_external", "EXTERNAL_SELF_MOD", `External input cannot modify protected path: "${filePath}" (matches "${protectedPath}")`);
                }
            }
            return null;
        },
 ...
 */
export function createAuthorityRules() {
    return [
