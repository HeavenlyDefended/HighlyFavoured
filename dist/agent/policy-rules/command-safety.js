...
export function getForbiddenCommandMatch(command) {
...
function createForbiddenPatternsRule() {
    return {
        ...
        evaluate(request) {
            ...
            if (match) {
                return deny("command.forbidden_patterns", "FORBIDDEN_COMMAND", `Blocked: ${match.description} (pattern: ${match.pattern})`);
            }
            return null;
        },
    };
