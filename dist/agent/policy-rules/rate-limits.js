 ...
 */
function countRecentDecisions(db, toolName, windowMs) {
    ...
    const row = db
       ...
       WHERE tool_name = ? AND decision = 'allow' AND created_at >= ?`)
        .get(toolName, cutoffStr);
    return row.count;
}
/**
 * Maximum 1 genesis prompt change per day.
 */
function createGenesisPromptDailyRule() {
    return {
        ...
        evaluate(request) {
            ...
            const db = request.context.db?.raw ?? request.context.rawDb;
            if (!db)
                return deny(this.id, "DB_UNAVAILABLE", "Rate limit check failed: database not accessible");
            const oneDayMs = 24 * 60 * 60 * 1000;
            const recentCount = countRecentDecisions(db, "update_genesis_prompt", oneDayMs);
            if (recentCount >= 1) {
                return deny("rate.genesis_prompt_daily", "RATE_LIMIT_GENESIS", `Genesis prompt change rate exceeded: ${recentCount} changes in the last 24 hours (max 1/day)`);
            }
            return null;
        },
    };
 ...
 */
function createSelfModHourlyRule() {
    return {
        ...
        evaluate(request) {
            ...
            if (recentCount >= 10) {
                return deny("rate.self_mod_hourly", "RATE_LIMIT_SELF_MOD", `Self-modification rate exceeded: ${recentCount} edits in the last hour (max 10/hour)`);
            }
            return null;
        },
    };
...
function createSpawnDailyRule() {
    return {
        ...
        evaluate(request) {
            ...
            if (recentCount >= 3) {
                return deny("rate.spawn_daily", "RATE_LIMIT_SPAWN", `Child spawn rate exceeded: ${recentCount} spawns in the last 24 hours (max 3/day)`);
            }
            return null;
        },
    };
