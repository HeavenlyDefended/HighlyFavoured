...
import { insertSpendRecord, getSpendByWindow, pruneSpendRecords, } from "../state/database.js";
/**
...
export class SpendTracker {
    ...
    getTotalSpend(category, since) {
        ...
        const row = this.db
            .prepare(`SELECT COALESCE(SUM(amount_cents), 0) as total FROM spend_tracking WHERE category = ? AND created_at >= ?`)
            .get(category, sinceStr);
        return row.total;
    }
    checkLimit(amount, category, limits) {
