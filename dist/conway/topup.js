...
export async function topupForSandbox(params) {
    ...
    if (chainType === "solana") {
        logger.info("Sandbox topup skipped: Solana wallets cannot use x402. Fund via Conway credits API or dashboard.");
        return null;
    }
...
export async function bootstrapTopup(params) {
    ...
    if (chainType === "solana") {
        if (creditsCents < creditThresholdCents) {
            logger.info("Bootstrap topup skipped: Solana wallets cannot use x402. Fund via Conway credits API or dashboard.");
        }
        return null;
    }
    if (creditsCents >= creditThresholdCents) {
