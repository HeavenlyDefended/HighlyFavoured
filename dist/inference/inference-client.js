...
class ProviderAttemptError extends Error {
    ...
    originalError;
    constructor(params) {
        const message = params.originalError instanceof Error
...
export class UnifiedInferenceClient {
    ...
    async executeWithRetries(resolved, params, requestedTier) {
        let retries = 0;
        while (true) {
            try {
                const result = await this.executeSingleRequest(resolved.client, resolved.provider.id, resolved.model, requestedTier, params);
    ...
    async executeSingleRequest(client, providerId, model, requestedTier, params) {
        ...
        if (params.stream) {
            ...
            const streamed = await this.consumeStreamResponse(stream);
    ...
    buildChatCompletionRequest(modelId, params) {
        const payload = {
            model: modelId,
    ...
    async consumeStreamResponse(stream) {
        let content = "";
        let usage = {
            inputTokens: 0,
            outputTokens: 0,
    ...
    markProviderFailure(providerId) {
        ...
        if (state.failures >= CIRCUIT_BREAKER_FAILURE_THRESHOLD) {
            ...
            this.registry.disableProvider(providerId, "circuit-breaker: too many consecutive inference failures", CIRCUIT_BREAKER_DISABLE_MS);
        }
