...
export class ProviderRegistry {
    providers;
    tierDefaults;
    disablements = new Map();
    emergencyStopCredits;
    constructor(providers, tierDefaults = DEFAULT_TIER_DEFAULTS, emergencyStopCredits = DEFAULT_EMERGENCY_STOP_CREDITS) {
    ...
    overrideBaseUrl(providerId, baseUrl) {
    ...
    resolveModel(tier, survivalMode = false) {
    ...
    resolveCandidates(tier, survivalMode = false) {
    ...
    disableProvider(id, reason, durationMs) {
    ...
    buildResolvedModel(provider, model) {
    ...
    applySurvivalTier(tier, survivalMode) {
        if (!survivalMode) {
            return tier;
        }
        if (tier === "reasoning") {
    ...
    assertEmergencyPolicy() {
        ...
        if (!plannerCall) {
            throw new Error(`Emergency stop active (${credits} credits < ${this.emergencyStopCredits}); only planner calls are allowed`);
        }
    }
}
...
function normalizeModels(input, fallbackModels) {
    if (!Array.isArray(input)) {
    ...
    for (const rawModel of input) {
        ...
        const fallback = fallbackModels.find((model) => model.id === id && model.tier === tier) ||
...
function normalizeTierDefault(input, fallback) {
    const preferredProvider = typeof input?.preferredProvider === "string" && input.preferredProvider.length > 0
        ? input.preferredProvider
        : fallback.preferredProvider;
    const fallbackOrder = Array.isArray(input?.fallbackOrder)
        ? input.fallbackOrder.filter((id) => typeof id === "string")
...
function deepCloneProvider(provider) {
    return {
        ...provider,
