...
export class AgentContextAggregator {
    aggregateChildUpdates(updates, budgetTokens) {
        const fullUpdates = [];
        ...
        for (const update of updates) {
            ...
            else if (matchesAny(update.status, PROGRESS_PATTERNS) ||
        ...
        const summaryEntries = [...groupedSummaries.entries()].map(([group, value]) => ({
            group,
            count: value.count,
    ...
    triageUpdate(update) {
        if (this.isError(update))
            return "full";
        if (this.isLargeFinancialEvent(update))
            return "full";
        if (this.isBlocked(update))
            return "full";
        if (this.isCompleted(update))
            return "summary";
        if (this.isProgress(update))
            return "summary";
        if (this.isHeartbeat(update))
            return "count";
        return "summary";
    }
    renderSummary(params) {
        ...
        if (params.summaryEntries.length > 0) {
            ...
            for (const entry of params.summaryEntries) {
                ...
                lines.push(`- ${entry.group}: ${entry.count} updates (${entry.completed} completed, ${entry.progress} progress, ${entry.other} other)${highlight}`);
            }
        }
        if (params.heartbeatCount > 0) {
            lines.push(`Heartbeat-only updates: ${params.heartbeatCount} agents alive.`);
        }
        lines.push(`Triage counts: full=${params.triageCounts.full}, summary=${params.triageCounts.summary}, count=${params.triageCounts.count}.`);
    ...
    isError(update) {
        if (typeof update.error === "string" && update.error.trim().length > 0)
            return true;
        return (matchesAny(update.status, ERROR_PATTERNS) ||
            matchesAny(update.kind, ERROR_PATTERNS) ||
            matchesAny(update.message, ERROR_PATTERNS));
    }
    isLargeFinancialEvent(update) {
        if (typeof update.budgetImpactPercent === "number") {
            return update.budgetImpactPercent > 10;
        }
        if (typeof update.financialAmount === "number" &&
    ...
    isBlocked(update) {
        if (update.blocked === true)
            return true;
        return (matchesAny(update.status, BLOCKED_PATTERNS) ||
            matchesAny(update.kind, BLOCKED_PATTERNS) ||
            matchesAny(update.message, BLOCKED_PATTERNS));
    }
    isCompleted(update) {
        return (matchesAny(update.status, COMPLETED_PATTERNS) ||
            matchesAny(update.kind, COMPLETED_PATTERNS));
    }
    isProgress(update) {
        return (matchesAny(update.status, PROGRESS_PATTERNS) ||
            matchesAny(update.kind, PROGRESS_PATTERNS));
    }
    isHeartbeat(update) {
        return (matchesAny(update.status, HEARTBEAT_PATTERNS) ||
...
function normalizeMessage(input, maxChars = 120) {
    if (!input)
        return null;
...
//# sourceMappingURL=agent-context-aggregator.js.map