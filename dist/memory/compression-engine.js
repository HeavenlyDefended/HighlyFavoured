...
import { estimateTokens } from "./event-stream.js";
...
export class CompressionEngine {
    ...
    inference;
    totalCheckpoints = 0;
    totalEmergencyTruncations = 0;
    compressedTurnCount = 0;
    compressionRatioSum = 0;
    peakUtilizationPercent = 0;
    turnsWithoutCompression = 0;
    constructor(contextManager, eventStream, knowledgeStore, inference) {
        this.contextManager = contextManager;
        this.eventStream = eventStream;
        this.knowledgeStore = knowledgeStore;
        this.inference = inference;
    }
    async evaluate(utilization) {
        this.peakUtilizationPercent = Math.max(this.peakUtilizationPercent, utilization.utilizationPercent);
        ...
        const olderThan5Ids = turnIds.slice(0, Math.max(0, turnIds.length - STAGE_4_KEEP_LAST_TURNS));
        ...
        const estimatedTokensSaved = this.estimateSavings(actions, turnEvents, utilization.usedTokens);
        if (actions.length === 0) {
    ...
    async execute(plan) {
        ...
        for (let stage = 1; stage <= plan.maxStage || (forceStage4 && stage <= 4); stage++) {
            ...
            try {
                if (stage === 1) {
                    const actions = plan.actions.filter((action) => action.type === "compact_tool_results");
                ...
                if (stage === 2) {
                    const actions = plan.actions.filter((action) => action.type === "compress_turns");
                    for (const action of actions) {
                ...
                if (stage === 3) {
                    const actions = plan.actions.filter((action) => action.type === "summarize_batch");
                    if (actions.length > 0) {
                ...
                if (stage === 4) {
                    const actions = plan.actions.filter((action) => action.type === "checkpoint_and_reset");
                ...
                if (stage === 5) {
                    const action = plan.actions.find((candidate) => candidate.type === "emergency_truncate");
                    totalSaved += await this.runStage5EmergencyTruncation(action?.keepLastN ?? STAGE_5_KEEP_LAST_TURNS);
                }
            }
            catch (error) {
    ...
    compactPrefixByTurnIds(turnIds, strategy) {
        if (turnIds.length === 0)
            return 0;
        const turnEvents = this.getTurnEvents();
    ...
    async runStage3BatchSummaries(actions) {
        ...
        for (const action of actions) {
            ...
            if (batchEvents.length === 0)
                continue;
            const rawTokens = batchEvents.reduce((sum, event) => sum + normalizeTokenCount(event), 0);
    ...
    async summarizeBatch(events, maxTokens) {
        const payload = events
            .map((event) => `[${event.createdAt}] (${event.type}) goal=${event.goalId ?? "-"} task=${event.taskId ?? "-"} ${normalizeContent(event.content)}`)
            .join("\n");
        const response = await this.inference.chat({
            ...
            messages: [
                {
                    role: "system",
                    content: "Summarize conversation events. Preserve agent IDs, financial amounts, task outcomes, and key decisions. Omit repetitive reasoning details.",
                },
                {
                    role: "user",
                    content: payload,
    ...
    async runStage4CheckpointAndReset(checkpointId) {
        const allEvents = this.getAllCompressionEvents();
        const turnEvents = this.getTurnEvents();
        const retainedTurnWindow = this.selectRetainedTurnWindow(turnEvents, STAGE_4_KEEP_LAST_TURNS);
        ...
        let summary;
        try {
            summary = await this.summarizeForCheckpoint(eventsBeforeBoundary, STAGE_4_SUMMARY_MAX_TOKENS);
        }
        catch {
    ...
    async summarizeForCheckpoint(events, maxTokens) {
        if (events.length === 0) {
        ...
        const response = await this.inference.chat({
            ...
            messages: [
                {
                    role: "system",
                    content: "Produce a checkpoint summary of the conversation. Preserve active goals/tasks, financial values, decisions, and outcomes. Be concise and actionable.",
                },
                {
                    role: "user",
    ...
    async rehydrateActiveTasks(activeTaskIds, events, agentAddress) {
    ...
    async runStage5EmergencyTruncation(keepLastN) {
        const allEvents = this.getAllCompressionEvents();
        const turnEvents = this.getTurnEvents();
        const retainedTurnWindow = this.selectRetainedTurnWindow(turnEvents, Math.max(1, keepLastN));
    ...
    async logCompressionError(stage, error) {
    ...
    async logCompressionMetrics(metrics) {
        this.eventStream.append({
            type: "compression",
            agentAddress: this.pickAgentAddress(this.getAllCompressionEvents()),
            goalId: null,
    ...
    estimateSavings(actions, turnEvents, usedTokens) {
    ...
    resolveBoundary(events, selectedIds) {
    ...
    collectActiveTasksAndGoals(events) {
        ...
        const activeGoalIds = [...new Set([...activeTasks.values()]
                .filter((goalId) => Boolean(goalId)))];
        return {
            activeTaskIds,
            activeGoalIds,
        };
    }
    extractKeyDecisions(events) {
        return events
            .filter((event) => event.type === "plan_updated" ||
            event.type === "action" ||
    ...
    selectRetainedTurnWindow(turnEvents, keepN) {
        if (turnEvents.length <= keepN)
            return [...turnEvents];
    ...
    getAllCompressionEvents() {
        const events = COMPRESSION_EVENT_TYPES.flatMap((type) => this.eventStream.getByType(type));
...
function resolveStage(utilizationPercent) {
    if (utilizationPercent > STAGE_5_THRESHOLD)
        return 5;
    if (utilizationPercent > STAGE_4_THRESHOLD)
        return 4;
    if (utilizationPercent > STAGE_3_THRESHOLD)
        return 3;
    if (utilizationPercent > STAGE_2_THRESHOLD)
        return 2;
    if (utilizationPercent > STAGE_1_THRESHOLD)
        return 1;
    return 1;
}
function normalizeTokenCount(event) {
    return event.tokenCount > 0 ? event.tokenCount : estimateTokens(event.content ?? "");
}
function normalizeContent(content, maxChars = 800) {
    const compact = (content ?? "").replace(/\s+/g, " ").trim();
    if (compact.length <= maxChars)
        return compact;
    return `${compact.slice(0, maxChars)}...`;
}
function plusOneMs(iso) {
...
function extractToolCallIds(content) {
    const ids = [];
...
function buildHeuristicSummary(events) {
    if (events.length === 0)
        return "No historical events available prior to checkpoint.";
