...
export class EpisodicMemoryManager {
    ...
    record(entry) {
        ...
        try {
            this.db.prepare(`INSERT INTO episodic_memory (id, session_id, event_type, summary, detail, outcome, importance, embedding_key, token_count, classification)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(id, entry.sessionId, entry.eventType, entry.summary, entry.detail ?? null, entry.outcome ?? null, entry.importance ?? 0.5, entry.embeddingKey ?? null, tokenCount, entry.classification ?? "maintenance");
        }
        catch (error) {
    ...
    getRecent(sessionId, limit = 10) {
        try {
            const rows = this.db.prepare("SELECT * FROM episodic_memory WHERE session_id = ? ORDER BY created_at DESC LIMIT ?").all(sessionId, limit);
    ...
    search(query, limit = 10) {
        try {
            ...
            const rows = this.db.prepare(`SELECT * FROM episodic_memory
         WHERE summary LIKE ? ESCAPE '\\' OR detail LIKE ? ESCAPE '\\'
         ORDER BY importance DESC, created_at DESC
         LIMIT ?`).all(`%${escaped}%`, `%${escaped}%`, limit);
            return rows.map(deserializeEpisodic);
        }
        catch (error) {
    ...
    markAccessed(id) {
        try {
            this.db.prepare("UPDATE episodic_memory SET accessed_count = accessed_count + 1, last_accessed_at = datetime('now') WHERE id = ?").run(id);
        }
        catch (error) {
    ...
    prune(retentionDays) {
        if (retentionDays <= 0)
            return 0;
        try {
            const result = this.db.prepare("DELETE FROM episodic_memory WHERE created_at < datetime('now', ?)").run(`-${retentionDays} days`);
    ...
    summarizeSession(sessionId) {
        try {
            const entries = this.db.prepare("SELECT * FROM episodic_memory WHERE session_id = ? ORDER BY created_at ASC").all(sessionId);
            if (entries.length === 0)
            ...
            if (successes > 0)
                summaryLines.push(`${successes} successful outcome(s).`);
            if (failures > 0)
                summaryLines.push(`${failures} failed outcome(s).`);
            if (strategic > 0)
                summaryLines.push(`${strategic} strategic decision(s).`);
...
function deserializeEpisodic(row) {
    return {
        id: row.id,
