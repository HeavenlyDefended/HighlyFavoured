 ...
 */
export function normalizeErrorType(error) {
...
export class MemoryIngestionPipeline {
    ...
    knowledgeStore;
    eventStream;
    enhancementsChecked = false;
    hasKnowledgeStoreTable = false;
    hasEventStreamTable = false;
    constructor(db) {
        this.db = db;
    ...
    extractMarketSignals(toolCalls) {
    ...
    updateKnowledgeStore(knowledgeStore, facts) {
        ...
        for (const fact of facts) {
            ...
            const exactMatch = existing.find((entry) => this.normalizeValue(entry.content) === normalizedValue);
            if (exactMatch) {
    ...
    detectContradictions(newFact, existing) {
        const contradictions = [];
        const newFactKey = this.normalizeKey(newFact.key);
        const newFactValue = this.normalizeValue(newFact.value);
        for (const entry of existing) {
            if (this.normalizeKey(entry.key) !== newFactKey)
                continue;
            if (this.normalizeValue(entry.content) === newFactValue)
                continue;
            contradictions.push({
                existingEntry: entry,
                newFact,
                description: `Conflicting values for "${newFact.key}": ` +
                    `"${entry.content}" (existing, ${entry.source}) vs "${newFact.value}" (new, ${newFact.source})`,
            });
        }
        return contradictions;
    }
    scoreConfidence(source, recency) {
        const sourceLower = source.toLowerCase();
        let sourceScore = 0.55;
        if (/official|docs|documentation|registry|conway|api|github\.com|wikipedia|\.gov\b|\.edu\b/.test(sourceLower)) {
            sourceScore = 0.9;
        }
    ...
    ingestKnowledgeEnhancements(sessionId, toolCallResults) {
        try {
            this.ensureEnhancementTableState();
            if (!this.hasKnowledgeStoreTable)
                return;
    ...
    toExtractedFact(signal) {
        const category = signal.type === "pricing" ? "financial" : "market";
        return {
            category,
    ...
    emitMarketSignals(sessionId, marketSignals) {
        if (!this.hasEventStreamTable)
            return;
    ...
    emitContradictions(sessionId, contradictions) {
        if (!this.hasEventStreamTable)
            return;
    ...
    resolveRecency(tc) {
        ...
        if (/\byesterday\b/i.test(tc.result))
            return "yesterday";
        if (/\btoday\b/i.test(tc.result))
            return "today";
        return new Date().toISOString();
    }
    splitSignalCandidates(result) {
    ...
    classifySignalTypes(fragment) {
        const types = [];
        const lower = fragment.toLowerCase();
        if (/\$[\d,.]+|\b\d+(?:\.\d+)?\s*(?:usd|usdc|cents)\b|\((?:\$\d+(?:\.\d{1,2})?\/yr)\)/i.test(fragment) ||
            /\b(price|pricing|cost|fee|subscription|rate|revenue)\b/.test(lower)) {
            types.push("pricing");
        }
        if (/\b(demand|trend|trending|growth|growing|volume|traffic|adoption|users?|waitlist|backlog|orders?)\b/.test(lower)) {
            types.push("demand");
        }
        if (/\b(competitor|competitors|rival|rivals|alternative|alternatives|vs\.?|market leader)\b/.test(lower)) {
    ...
    scoreRecency(recency) {
        ...
        if (normalized.includes("yesterday"))
            return 0.95;
        const relativeMatch = normalized.match(/(\d+)\s*(minute|hour|day|week|month|year)s?\s*ago/);
        if (relativeMatch) {
