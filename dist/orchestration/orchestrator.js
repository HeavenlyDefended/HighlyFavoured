...
import { goalToPlannerInput, planGoal, replanAfterFailure, taskToPlannerFailureInput, } from "./planner.js";
...
export class Orchestrator {
    ...
    async handlePlanningPhase(state) {
        ...
        let output;
        try {
            output = await planGoal(goalToPlannerInput(goalRowToGoal(goal)), await buildPlannerContext({
                db: this.params.db,
    ...
    async handleReplanningPhase(state) {
        ...
        let output;
        try {
            output = await replanAfterFailure(goalToPlannerInput(goalRowToGoal(goal)), taskToPlannerFailureInput(taskRowToTaskNode(failedTaskRow)), await buildPlannerContext({
                db: this.params.db,
                workspace: new AgentWorkspace(goal.id),
                funding: this.params.funding,
                identityAddress: this.params.identity.address,
    ...
    async persistPlannerOutput(goalId, output, mode) {
        const canonicalKey = `orchestrator.plan.${goalId}`;
        const modeKey = `orchestrator.${mode}.${goalId}`;
        this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(canonicalKey, JSON.stringify(output));
        if (modeKey !== canonicalKey) {
            this.params.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(modeKey, JSON.stringify(output));
        }
        try {
