...
export class PlanModeController {
    ...
    transition(from, to, reason) {
        ...
        if (state.phase !== from) {
            throw new Error(`Invalid transition precondition: state is '${state.phase}', expected '${from}' (reason: ${reason})`);
        }
    ...
    setState(state) {
        ...
        this.db.prepare("INSERT OR REPLACE INTO kv (key, value, updated_at) VALUES (?, ?, datetime('now'))").run(PLAN_MODE_STATE_KEY, JSON.stringify(next));
    }
}
export async function classifyComplexity(params, inference) {
    ...
    try {
        ...
        const estimatedSteps = clampSteps(typeof parsed?.estimatedSteps === "number"
...
export async function loadPlan(planFilePath) {
...
export async function reviewPlan(plan, config) {
...
export function shouldReplan(state, trigger) {
...
function sanitizeExecutionState(value, fallback) {
    ...
    const spawnedAgentIds = Array.isArray(record.spawnedAgentIds)
        ? record.spawnedAgentIds.filter((entry) => typeof entry === "string")
        : fallback.spawnedAgentIds;
    const replansRemaining = toNonNegativeInteger(record.replansRemaining, fallback.replansRemaining);
    const enteredAt = typeof record.phaseEnteredAt === "string" && record.phaseEnteredAt.trim().length > 0
...
function asExecutionPhase(value) {
    if (value === "idle" ||
        value === "classifying" ||
        value === "planning" ||
        value === "plan_review" ||
...
function normalizeStepOutline(value, description, estimatedSteps) {
...
function heuristicStepOutline(description, estimatedSteps) {
...
function estimateStepsHeuristic(taskDescription, availableTools) {
...
function normalizeApprovalConfig(config) {
    const mode = config.mode === "auto" || config.mode === "supervised" || config.mode === "consensus"
        ? config.mode
        : "auto";
    return {
        mode,
        ...
        consensusCriticRole: typeof config.consensusCriticRole === "string" && config.consensusCriticRole.trim().length > 0
...
function renderPlanMarkdown(goalId, version, plan) {
    const lines = [];
    ...
    if (plan.tasks.length === 0) {
        ...
        for (let index = 0; index < plan.tasks.length; index += 1) {
            ...
            lines.push(`${index + 1}. [ ] ${task.title} — role: ${task.agentRole}, deps: ${task.dependencies.join(",") || "none"}, cost: ${task.estimatedCostCents}c, timeout: ${task.timeoutMs}ms`);
