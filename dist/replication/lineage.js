...
export function getLineage(db) {
    ...
    const alive = children.filter((c) => c.status === "running" || c.status === "sleeping" || c.status === "healthy").length;
 ...
 */
export async function pruneDeadChildren(db, cleanup, keepLast = 5) {
    const children = db.getChildren();
    const dead = children.filter((c) => c.status === "dead" || c.status === "failed" || c.status === "stopped");
 ...
 */
export async function refreshChildrenStatus(conway, db, healthMonitor) {
    if (healthMonitor) {
