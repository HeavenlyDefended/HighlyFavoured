...
export async function spawnChild(conway, identity, db, genesis, lifecycle) {
    ...
    try {
        ...
        const genesisJson = JSON.stringify({
            ...
            chainType: genesis.chainType || identity.chainType || "evm",
        }, null, 2);
        ...
        const parentChainType = identity.chainType || "evm";
...
async function spawnChildLegacy(conway, identity, db, genesis, childId) {
    ...
    try {
        ...
        const initResult = await childConway.exec("node /root/automaton/dist/index.js --init 2>&1", 60_000);
        const legacyParentChainType = genesis.chainType || identity.chainType || "evm";
