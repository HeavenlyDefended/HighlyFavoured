...
async function askString(label, current, required = false) {
...
async function askRequiredString(label, current) {
    const result = await askString(label, current, true);
    return result ?? current;
}
/** Prompt for a number. Enter = keep current. */
async function askNumber(label, current) {
    const raw = await ask(`  ${chalk.white("→")} ${label} ${chalk.dim("[" + current + "]")}${chalk.dim(" (Enter to keep)")}: `);
    if (raw === "")
        return current;
...
async function askBool(label, current) {
    const display = current ? chalk.green("yes") : chalk.dim("no");
    const raw = await ask(`  ${chalk.white("→")} ${label} ${chalk.dim("[")}${display}${chalk.dim("]")}${chalk.dim(" (y/n, Enter to keep)")}: `);
    if (raw === "")
        return current;
    if (raw === "y" || raw === "yes" || raw === "1" || raw === "true")
        return true;
    if (raw === "n" || raw === "no" || raw === "0" || raw === "false")
        return false;
...
function printModelTable(models, currentModelId) {
    ...
    for (let i = 0; i < models.length; i++) {
        ...
        console.log(`  ${chalk.white(num + ".")} ${chalk.cyan(m.modelId.padEnd(36))} ${chalk.dim(provider)} ${cost}${tools}${active}`);
    }
}
async function pickFromList(label, current, models) {
    if (models.length === 0) {
    ...
    console.log("");
    const raw = await ask(`  ${chalk.white("→")} Enter number ${chalk.dim("(Enter to keep " + current + ")")}: `);
    if (raw === "")
        return current;
...
async function configureProviders(config) {
    ...
    config.conwayApiKey = await askRequiredString("Conway API key", config.conwayApiKey);
...
async function configureModelStrategy(config) {
    console.log(chalk.cyan("\n  ── Model Strategy ──────────────────────────────\n"));
    ...
    s.hourlyBudgetCents = await askNumber("Hourly inference budget (cents, 0 = unlimited)", s.hourlyBudgetCents);
    s.sessionBudgetCents = await askNumber("Session inference budget (cents, 0 = unlimited)", s.sessionBudgetCents);
    s.perCallCeilingCents = await askNumber("Per-call ceiling (cents, 0 = unlimited)", s.perCallCeilingCents);
...
async function configureTreasury(config) {
    console.log(chalk.cyan("\n  ── Treasury Policy ─────────────────────────────\n"));
    ...
    t.requireConfirmationAboveCents = await askNumber("Require confirmation above", t.requireConfirmationAboveCents);
    config.treasuryPolicy = t;
    console.log("");
}
// ─── Section: General ─────────────────────────────────────────────
async function configureGeneral(config) {
    console.log(chalk.cyan("\n  ── General ─────────────────────────────────────\n"));
    config.name = await askRequiredString("Agent name", config.name);
    config.logLevel = await askChoice("Log level", ["debug", "info", "warn", "error"], config.logLevel);
