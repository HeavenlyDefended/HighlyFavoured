 ...
 */
export function createSocialClient(relayUrl, account, db) {
    ...
    function checkReplayNonce(nonce) {
        if (!db)
            return false;
        try {
            const row = db
            ...
            if (row)
                return true; // Already seen this nonce
            // Insert nonce with 5 min TTL
            const expiresAt = new Date(Date.now() + MESSAGE_LIMITS.replayWindowMs).toISOString();
            db.prepare("INSERT OR IGNORE INTO heartbeat_dedup (dedup_key, task_name, expires_at) VALUES (?, ?, ?)").run(`social:nonce:${nonce}`, "social_replay", expiresAt);
            return false;
        }
        catch {
            return false;
    ...
    return {
        ...
        unreadCount: async () => {
            ...
            if (!res.ok) {
                const err = await res.json().catch(() => ({ error: res.statusText }));
                throw new Error(`Unread count failed (${res.status}): ${err.error || res.statusText}`);
            }
            const data = (await res.json());
            return data.unread;
        },
